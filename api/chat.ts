...
// Inicializamos Gemini de forma segura usando la variable de entorno de Vercel
...
export default async function handler(req: any, res: any) {
  // Control de método HTTP (reemplaza la lógica que hacía Express Router)
  if (req.method !== "POST") {
  ...
  try {
    ...
    if (!ai) {
      return res.status(500).json({
        ...
        message: "Para chatear con los tutores de IA se requiere la clave de API de Gemini. Por favor, añádela en la sección correspondiente de Vercel."
      });
    }
    ...
    // Configuración por defecto del tutor filológico
...
`;
    ...
    // Mapeamos el historial al formato estricto que exige Gemini
    const contents = messages.map((m: any) => ({
      role: m.role === "user" ? "user" : "model",
      parts: [{ text: m.text || m.content || "" }]
    }));

    // Llamada oficial utilizando el SDK moderno y el modelo recomendado
